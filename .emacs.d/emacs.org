...
The first path we set is =fc:base-dir=.  This is where all the files for Emacs are.
#+BEGIN_SRC emacs-lisp
  (defvar fc:base-dir (file-name-directory load-file-name)
    "The root dir of the Emacs distribution.")
#+END_SRC

There's a few personal files that we will need later (like encrypted files containing passwords, or bookmarks, etc).  This files are stored in a directory named =personal= inside =fc:base-dir=.
#+BEGIN_SRC emacs-lisp
